...
import { getFilterPlaceholder, rolloutStateColor } from "../../util";
import ECSClusterServiceTasks from "./ECSClusterServiceTasks";
import ECSServiceDeployments from "./ECSServiceDeployments";
    ...
    <List isLoading={isLoading} searchBarPlaceholder={getFilterPlaceholder("services")} isShowingDetail={true}>
      {services ? (
        services.map((service) => {
          const activeRollout = service.deployments?.find((deployment) => deployment.rolloutState === "IN_PROGRESS");
            ...
            <List.Item
                ...
                <List.Item.Detail
                  ...
                  metadata={
                    <List.Item.Detail.Metadata>
                      ...
                      <List.Item.Detail.Metadata.Label
                        title="Creation Date"
                        text={service.createdAt?.toLocaleString()}
                      />
                      <List.Item.Detail.Metadata.Label
                        title="Providers"
              ...
              actions={
                <ActionPanel>
                  ...
                  <Action.Push
                    ...
                    icon={Icon.Rocket}
                    target={<ECSServiceDeployments service={service} />}
                  />
                  <Action
                    icon={Icon.Repeat}
                    title="Force New Deployment"
                    onAction={() => forceNewDeployment(service)}
                  />
              ...
              accessories={[
                ...(activeRollout
                  ? [
                      {
                        tag: { value: "Rolling Out", color: rolloutStateColor(activeRollout.rolloutState) },
                        tooltip: activeRollout.rolloutStateReason || "Deployment in progress",
                      },
                    ]
                  : []),
